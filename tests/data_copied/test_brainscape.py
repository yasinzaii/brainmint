...
@pytest.mark.parametrize("dataset_cfg", ["dataset/brainscape_test"]) # relative to CONFIG_DIR
def test_dataset_builds_and_loads(dataset_cfg, num_workers):

    # Hydra needs an absolute path
    with initialize_config_dir(str(CONFIG_DIR), version_base=None):
        
        # Overrides
        overrides=[
                f"dataset.brainscape.subset_frac={0.001}",
                f"dataset.brainscape.num_workers={num_workers}",
        ]
        
        # Composing 
        cfg = compose(config_name=dataset_cfg, overrides=overrides)
...
def test_modality_class_labels_and_mapping(dataset_cfg="dataset/brainscape_test", num_workers=0):
    with initialize_config_dir(str(CONFIG_DIR), version_base=None):
