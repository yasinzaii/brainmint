...
class ControlNet(MONAIControlNet):
    """
    Generic ControlNet implementation for diffusion models based on Zhang and Agrawala
    "Adding Conditional Control to Text-to-Image Diffusion Models" (https://arxiv.org/abs/2302.05543).
    Built on MONAI's ControlNet implementation.

    This class overrides ``__init__`` to attach optional synthetic-modality conditioning while still
    delegating the core architecture setup to ``MONAIControlNet``.
    ...
    Args:
        ...
        synthetic_cond_dim: number of channels for optional global conditioning embedding (0 disables the embedding).
        use_synthetic_embedding: if False, skip the optional conditioning even if synthetic_cond_dim > 0.
    ...
    def __init__(
        ...
        synthetic_cond_dim: int = 0,
        use_synthetic_embedding: bool = True,
        ...
        additional_embedding_dim = time_embed_dim if use_synthetic_embedding and synthetic_cond_dim > 0 else 0
        super().__init__(
        ...
        self.synthetic_cond_dim = int(synthetic_cond_dim)
        self.use_synthetic_embedding = bool(use_synthetic_embedding)
        self.conditioning_embedding_in_channels = conditioning_embedding_in_channels

        self.synthetic_embedding: nn.Module | None
        if self.use_synthetic_embedding and self.synthetic_cond_dim > 0:
            if additional_embedding_dim <= 0:
                raise ValueError("additional_embedding_dim must be > 0 when synthetic embedding is enabled.")
            self.synthetic_embedding = nn.Sequential(
                nn.Linear(self.synthetic_cond_dim, additional_embedding_dim),
                nn.SiLU(),
                nn.Linear(additional_embedding_dim, additional_embedding_dim),
            )
        else:
            self.synthetic_embedding = None
    ...
    def forward(
        ...
        if self.synthetic_embedding is not None:
            if is_mod_synthetic is None:
                raise ValueError(
                    "ControlNet was created with synthetic conditioning enabled "
                    "but no `is_mod_synthetic` was provided to forward()."
                )
            syn = is_mod_synthetic.to(dtype=emb.dtype)
            if syn.ndim != 2 or syn.shape[1] != self.synthetic_cond_dim:
                raise ValueError(
                    "is_mod_synthetic must have shape (B, 1, synthetic_cond_dim) when synthetic embedding is enabled."
                )
            emb = torch.cat((emb, self.synthetic_embedding(syn)), dim=1)
