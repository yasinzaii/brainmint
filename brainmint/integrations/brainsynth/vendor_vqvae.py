# Copied from https://github.com/AmigoLab/BrainSynth/blob/05adc8cf3a248a1d7d7a55f4d0111373dc90d5eb/src/networks/vqvae/vqvae.py
import abc
...
class VQVAEBase(nn.Module, metaclass=abc.ABCMeta):
    ...
    @abc.abstractmethod
    def set_ema_decay(
        self, decay: Sequence[float] | float, index: int = None
    ) -> Sequence[float]:
        raise NotImplementedError
    ...
    def set_commitment_cost(
        self, commitment_factor: Sequence[float] | float
    ) -> Sequence[float]:
        raise NotImplementedError
...
# Copied from https://github.com/AmigoLab/BrainSynth/blob/05adc8cf3a248a1d7d7a55f4d0111373dc90d5eb/src/networks/vqvae/baseline.py
...
class Quantizer(nn.Module):
    ...
    def forward(self, x):
        ...
        def compute_avg_probs(e):
            return torch.histc(e.float(), bins=self.n_embed, max=self.n_embed).float().div(e.numel())

        def compute_perplexity(probs):
            return torch.exp(-torch.sum(probs * torch.log(probs + 1e-10)))

        self.perplexity_code = compute_perplexity(compute_avg_probs(embed_idx))
...
class BaselineVQVAE(VQVAEBase, nn.Module):
    ...
    def construct_decoder(self) -> nn.ModuleList:
        ...
        if self.output_act:
            modules.append(Act[self.output_act]())
