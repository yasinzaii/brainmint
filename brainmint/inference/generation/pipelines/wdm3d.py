...
class WDM3DGenerationPipeline(DiffusionPipeline):
    """Generation pipeline for WDM-3D."""

    required_modules: ClassVar[Set[str]] = {"wdm3d"}
    ...
    def run(self, batch: Mapping[str, Any], *, ctx: InferenceContext) -> Dict[str, Any]:
        model = ctx.get("wdm3d", required=True)
        if not hasattr(model, "sample"):
            raise TypeError("Context module 'wdm3d' must expose a .sample(...) method.")
        ...
        if not isinstance(sample, torch.Tensor) or sample.dim() != 5:
            raise ValueError(
                "WDM3D pipeline expected 5D tensor (B,1,Z,Y,X), "
